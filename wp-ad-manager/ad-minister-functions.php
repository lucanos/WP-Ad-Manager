...
function administer_load_widgets () {
	...
	foreach ($positions as $position) {
		if ($position['type'] == 'widget' && str_replace(' ', '_', preg_replace('/[^a-zA-Z0-9\s]/', '', $position['position'])) ) {
			if(!class_exists($position['position']))
				eval('
				class ' . $position['position'] . ' extends WP_Widget {
					function ' . $position['position'] . '() {
						parent::WP_Widget(false, $name = "' . $position['position'] . '");
					}
				...
				register_widget(' . $position['position'] . ');
				');
		}
	}
}
